...
public class BizManager {
					...
					if (jsonStr.length() > 0) {
						...
						try {
							...
							if (status == 1) {
								...
								ApiConfigs.selectId = jsonObj
										.getString("musicId");
								ApiConfigs.selectName = jsonObj
										.getString("musicName");
							}
	 ...
	 * @param handler
	 */
	public void getMySongList(final Context context, final int page,
			final int size, final Handler handler) {
			...
			public void run() {
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils.getRequest(
							ApiConfigs.requestLink + "getMySongJsonList",
							params);
					if (jsonStr.length() > 0) {
	 ...
	 * @param handler
	 */
	public void getAllSongList(final Context context, final int page,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils.getRequest(
							ApiConfigs.requestLink + "getAllSongJsonList",
							params);
					if (jsonStr.length() > 0) {
						mixSongList = JsonUtils
								.getMixSongList(context, jsonStr);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils.getRequest(ApiConfigs.musictLink
	 ...
	 * @param audioFile
	 */
	public void updateFileToMix(final Context context,
			final ProgressBar progressBar, final String musicId,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				HttpUploadPost post = new HttpUploadPost(context, progressBar,
						urlStr, appid, appkey, speechfmt, musicId, phoneKey,
						audioFile, handler);
				post.execute();
				Looper.loop();
			}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils.getRequest(
							ApiConfigs.requestLink + "compoundSongAgain",
							params);
					if (jsonStr.length() > 0) {
						JSONObject jsonObj;
						System.out
								.println(">>>>>>>>>>---------reMixMusic重新合成请求："
										+ jsonStr);
						try {
							jsonObj = new JSONObject(jsonStr);
							int status = jsonObj.getInt("status");
							...
							if (status == 1) {
								...
								ApiConfigs.speechid = jsonObj
										.getString("speechid");
								ApiConfigs.recordId = jsonObj
										.getString("recordId");
								checkMixMusic(context, ApiConfigs.songId,
										"amr", handler);
							} else {
								handler.obtainMessage(status, msg)
										.sendToTarget();
							}
						} catch (JSONException e) {
							// TODO Auto-generated catch block
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils
							.getRequest(ApiConfigs.requestLink
									+ "getSongResStatus", params);
					if (jsonStr.length() > 0) {
						JSONObject jsonObj;
						System.out
								.println(">>>>>>>>>>---------checkMixMusic合成音乐查询："
										+ jsonStr);
						try {
							jsonObj = new JSONObject(jsonStr);
							int status = jsonObj.getInt("status");
							String msg = jsonObj.getString("msg");
							if (status == 1) {
								ApiConfigs.songId = jsonObj
										.getString("songUrl");
								ApiConfigs.tempId = jsonObj.getString("tempId");
								handler.obtainMessage(1, ApiConfigs.songId)
										.sendToTarget();
							} else if (status == 19) {
								Thread.sleep(500);
								checkMixMusic(context, songId, speechfmt,
										handler);
							} else {
								handler.obtainMessage(status, msg)
										.sendToTarget();
	...
	public void setPraise(final Context context, final String songId,
	 ...
	 * @param handler
	 */
	public void saveReName(final Context context, final String songName,
			final Handler handler) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils.getRequest(
							ApiConfigs.requestLink + "save", params);
					if (jsonStr.length() > 0) {
						JSONObject jsonObj;
						System.out.println(">>>>>>>>>>---------save保存并改名："
								+ jsonStr);
						try {
							jsonObj = new JSONObject(jsonStr);
							int status = jsonObj.getInt("status");
	 ...
	 * @param handler
	 */
	public void delete(final Context context, final String songId,
			final Handler handler) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (HttpUtils.isOpenNetwork(context) == true) {
					...
					String jsonStr = HttpUtils.getRequest(
							ApiConfigs.requestLink + "delete", params);
					if (jsonStr.length() > 0) {
						JSONObject jsonObj;
						System.out.println(">>>>>>>>>>---------delete删除用户歌曲："
								+ jsonStr);
						try {
							jsonObj = new JSONObject(jsonStr);
							int status = jsonObj.getInt("status");
