package com.mixmusic.listview.refresh;
...
import com.mixmusic.listview.refresh.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener{
	...
	private static final int WHAT_ON_REFRESH = 2;		// Handler what 刷新�?
	private static final int WHAT_DID_REFRESH = 3;		// Handler what 已经刷新�?
	private static final int WHAT_SET_HEADER_HEIGHT = 4;// Handler what 设置高度
	private static final int WHAT_DID_MORE = 5;			// Handler what 已经获取完更�?
	
	private static final int DEFAULT_HEADER_VIEW_HEIGHT = 105;	// 头部文件原本的高�?
	...
	private float mMotionDownLastY;	// 按下时�?的Y轴坐�?
	
	private boolean mIsDown;			// 是否按下
	private boolean mIsRefreshing;		// 是否下拉刷新�?
	private boolean mIsFetchMoreing;	// 是否获取更多�?
	...
	/**
	 * 是否�?��自动获取更多
	 * 自动获取更多，将会隐藏footer，并在到达底部的时�?自动刷新
	 * @param index 倒数第几个触�?
	 */
	...
	private void initHeaderViewAndFooterViewAndListView(Context context){
		 ...
		 * 如果要修改，和它相关的设置都要更�?
		 */
		mHeaderView = LayoutInflater.from(context).inflate(R.layout.listview_refresh_header, null);
		 ...
		 * 自定义脚部文�?
		 */
		mFooterView = LayoutInflater.from(context).inflate(R.layout.listview_loader_footer, null);
		...
		/*
		 * ScrollOverListView 同样是�?虑到都是使用，所以放在这�?
		 * 同时因为，需要它的监听事�?
		 */
